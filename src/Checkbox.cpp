...
namespace DapperUI
{
	Checkbox::Checkbox (int bx, int by, int size, int ot)
	{
		...
		idleColor.r = 255, idleColor.g = 0, idleColor.b = 0, idleColor.a = 255;
	...
	void Checkbox::handleEvent(const SDL_Event& event)
	{
		...
		if (const bool isMouseTouching = SDL_PointInRect(&mousePos, &rect); isMouseTouching && event.type == SDL_MOUSEBUTTONDOWN)
		{
			animationTimer = 0.0f;
			swapColors();
